...
<head>
  ...
  <title>Trip Tricks | @yield('title')</title>
...
<div class="wrapper">
      ...
      <li class="nav-item dropdown user-menu">
        ...
        <ul class="dropdown-menu dropdown-menu-lg dropdown-menu-right">
          ...
          <li class="user-header bg-primary">
            ...
            <p>
              {{auth()->user()->name}}
              <small>{{__('Member since')}} {{auth()->user()->created_at->toFormattedDateString()}}</small>
            </p>
          </li>
          <!-- Menu Footer-->
          <li class="user-footer">
            <a href="{{route('profile')}}" class="btn btn-default btn-flat">{{__('Profile')}}</a>
            <a href="{{ route('logout') }}" onclick="event.preventDefault(); document.getElementById('logout-form').submit();" class="btn btn-default btn-flat float-right">{{__('Sign out')}}</a>
  ...
  <aside class="main-sidebar sidebar-dark-primary elevation-4">
    ...
    <a href="{{ URL::to('/') }}" class="brand-link">
      ...
      <span class="brand-text font-weight-light">{{__('Trip Tricks')}}</span>
    </a>
  ...
  <footer class="main-footer">
    <strong>Copyright &copy; {{date('Y')}} <a href="{{URL::to('/')}}">triptricks.com</a>.</strong>
    All rights reserved.
